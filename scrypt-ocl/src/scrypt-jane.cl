...
static void keccak_block_core(scrypt_hash_state *S) {
  uint2 t[5];
  uint2 u[5];
  uint2 v;
  uint2 w;
...
static void keccak_block(scrypt_hash_state *S, const uint4 *in4) {
  uint4 *s4 = S->state4;
  uint i;
...
static void keccak_block_zero(scrypt_hash_state *S, const uint4 *in4) {
  uint4 *s4 = S->state4;
  uint i;
...
static void scrypt_hash_update_72(scrypt_hash_state *S, const uint4 *in4) {
  /* handle the current data */
  keccak_block_zero(S, in4);
}

static void scrypt_hash_update_80(scrypt_hash_state *S, const uint4 *in4) {
...
static void scrypt_hash_update_128(scrypt_hash_state *S, const uint4 *in4) {
  const uchar *in = (const uchar *)in4;
  uint i;
...
static void scrypt_hash_update_4_after_128(scrypt_hash_state *S, uint in) {
  // leftover = 14
  /* handle the previous data */
...
static void scrypt_hash_update_64(scrypt_hash_state *S, const uint4 *in4) {
  uint i;
...
static void scrypt_hash_finish_80_after_64(scrypt_hash_state *S, uint4 *hash4) {
...
static void scrypt_hash_finish_80_after_80_4(scrypt_hash_state *S,
                                             uint4 *hash4) {
  uint i;
...
static void scrypt_hash_finish_80_after_128_4(scrypt_hash_state *S,
                                              uint4 *hash4) {
  // leftover = 15
...
static void scrypt_hmac_init(scrypt_hmac_state *st, const uint4 *key) {
...
static void scrypt_hmac_update_80(scrypt_hmac_state *st, const uint4 *m) {
  /* h(inner || m...) */
...
static void scrypt_hmac_update_4_after_128(scrypt_hmac_state *st, uint m) {
  /* h(inner || m...) */
  scrypt_hash_update_4_after_128(&st->inner, m);
}

static void scrypt_hmac_finish_128B(scrypt_hmac_state *st, uint4 *mac) {
  /* h(inner || m) */
  uint4 innerhash[4];
...
static void scrypt_hmac_finish_32B(scrypt_hmac_state *st, uint4 *mac) {
  /* h(inner || m) */
  uint4 innerhash[4];
...
static void scrypt_copy_hmac_state_128B(scrypt_hmac_state *dest,
                                        const scrypt_hmac_state *src) {
  uint i;
...
static void
scrypt_ChunkMix_inplace_Bxor_local(uint4 *restrict B /*[chunkWords]*/,
                                   uint4 *restrict Bxor /*[chunkWords]*/) {
  /* 1: X = B_{2r - 1} */
...
static void scrypt_ChunkMix_inplace_local(uint4 *restrict B /*[chunkWords]*/) {
  /* 1: X = B_{2r - 1} */
...
kernel void scrypt(private const uint N, private const ulong starting_index,
  ...
  password[0] = input[0];
  password[1] = input[1];
  password[2].x = convert_uint(index & 0xFFFFFFFF);
  password[2].y = convert_uint((index >> 32) & 0xFFFFFFFF);
  password[2].zw = 0;
  password[3] = 0;
  password[4] = 0;
  ...
  scrypt_pbkdf2_32B(password, X, &out4[gid * 2]);
}
